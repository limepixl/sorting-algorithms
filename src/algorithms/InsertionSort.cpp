...
void InsertionSort::runSort(std::vector<RectangleShape> arr, sf::RenderWindow* window, int delay) {
    int n = arr.size();
    for(int i = 0; i < n; i++) {
        for(int j = i; j > 0; j--) {
            if(arr.at(j-1).value > arr.at(j).value) {
                sf::Vector2f firstPos = arr.at(j-1).getPosition();
                sf::Vector2f secondPos = arr.at(j).getPosition();

                std::iter_swap(arr.begin() + j, arr.begin() + j - 1);

                arr.at(j-1).setPosition(firstPos);
                arr.at(j).setPosition(secondPos);
